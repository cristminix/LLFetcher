import { getDsRecordsByType, findProp } from "./fn"
...
class CourseApiLegacy {
  ...
  async getCourse(refresh = false) {
    ...
    if (rows.length > 0) {
      ...
      if (filteredRows.length > 0) {
        ...
        this.course = await mCourse.create(
          title,
          slug,
          course.duration,
          sourceCodeRepository,
        ...
        if (Array.isArray(exerciseFiles)) {
          exerciseFiles.map(async (item) => {
            ...
            const rec = await mExerciseFile.create(
              name,
              url,
              sizeInBytes,
              this.course.id
        ...
        if (row.primaryThumbnail) {
          ...
          if (Array.isArray(artifacts)) {
            for (const artifact of artifacts) {
              ...
              const thumbnail = await mThumbnail.create(
                this.course.id,
                size,
                url,
                expiresAt
    ...
    for (const sectionTmp of sectionsTmp) {
      ...
      if (this.ds[sectionUrn]) {
        if (this.ds[sectionUrn].__data) {
          ...
          const section = await mSection.create(
            sectionData.title,
            slugify(sectionData.title),
            this.course.id,
            itemStars
          )
        ...
        if (!tocsFoundInDb) {
          for (const itemStar of section.itemStars) {
            ...
            if (tocDataFound) {
              ...
              const toc = await mToc.create(
                title,
                slug,
                courseUrlFromSlug(`${this.course.slug}/${slug}`),
                duration,
                null,
                null,
                section.id,
  ...
  async getAuthors(refresh = false) {
    ...
    this.authors = null
    const results = getDsRecordsByType(
    ...
    if (results) {
      for (const authorData of results) {
        ...
        const rec = await mAuthor.create(
          author.name,
          slug,
          author.biography,
