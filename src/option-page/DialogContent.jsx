...
class DialogContent extends Component {
    constructor(props){
        super(props)
        this.btnOpenModelRef =  createRef(null)
        this.state = {
            message : "",
            title:"",
            mode: "alert"
    ...
    alert(title,message){
        this.setState({title, message, mode:"alert"})
        this.btnOpenModelRef.current.click()
    }
    ...
    return <>
    <button style={{display:'none'}} type="button" ref={this.btnOpenModelRef} className="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-lg border border-transparent font-semibold bg-white text-slate-800 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:bg-slate-800 dark:text-white dark:focus:ring-offset-gray-800" data-hs-overlay="#hs-basic-modal">
  Open modal
</button>
