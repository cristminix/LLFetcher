...
const QueueMan = forwardRef(({ store, config, activeSlug = null, onQueueAllFinished = (f) => f, logStatusBar = (f) => f, clearStatusBar = (f) => f }, ref) => {
  // ref=this
  // ref.current = this
  ...
  const handleQDlStatusMChange = (idx, state) => {
    setQDlStatusM((prevQDlStatusM) => prevQDlStatusM.map((n_qDlStatusM) => (n_qDlStatusM.idx === idx ? { ...n_qDlStatusM, state } : n_qDlStatusM)))
  }
  const handleQDlStatusTChange = (idx, state) => {
    setQDlStatusT((prevQDlStatusT) => prevQDlStatusT.map((n_qDlStatusT) => (n_qDlStatusT.idx === idx ? { ...n_qDlStatusT, state } : n_qDlStatusT)))
  }
  ...
  const processQueue_fetchTrans = async (qItem, qState, toc) => {
    ...
    if (transcripts.length == 0) {
      ...
      try {
        const tmpData = await processQueue_fetchMeta(qItem, qState, toc, false)
        console.log(tmpData)
      ...
      if (vttUrl) {
        try {
          success = await downloadVtt(
            ...
            (e, idx, course, toc, opt, t, provider) => onDownloadProgress(e, idx, course, toc, opt, qState, t, provider)
          )
        } catch (e) {
          console.error(e)
        }
  ...
  const processQueue_fetchMedia = async (qItem, qState, toc) => {
    ...
    if (slocs.length == 0) {
      nQState = await updateQState(qItem, qState, QueueState.FETCH_META_RETRY)
      let [slocs_, transcripts_, err] = await processQueue_fetchMeta(qItem, qState, toc, false)
      if (!err) {
      ...
      if (mediaUrl) {
        try {
          success = await downloadMedia(
            ...
            qState,
            downloadProviderRef.current,
            (e, idx, course, toc, opt, qState, t, provider) => onDownloadProgress(e, idx, course, toc, opt, qState, t, provider)
          )
  ...
  const processQueue_fetchMeta = async (qItem, qState, toc, returQState = true) => {
    let nQState = null
  ...
  const handleQdlViewsMChange = (idx, loaded, size) => {
    ...
    setQDlViewsM((prevQDlViewsM) => prevQDlViewsM.map((n_qDlViewsM) => (n_qDlViewsM.idx === idx ? { ...n_qDlViewsM, loaded, size, percentage } : n_qDlViewsM)))
  }
  const handleQdlViewsTChange = (idx, loaded, size) => {
    // Use map to create a new array with the updated user
    const percentage = Math.floor((loaded / size) * 100)

    setQDlViewsT((prevQDlViewsT) => prevQDlViewsT.map((n_qDlViewsT) => (n_qDlViewsT.idx === idx ? { ...n_qDlViewsT, loaded, size, percentage } : n_qDlViewsT)))
  }
  ...
  const confirmLeaveWidow = (e) => {
    ...
    var confirmationMessage = ("Are you sure you want to leave?"(e || window.event).returnValue = confirmationMessage) // For some older browsers
              ...
              <div className="flex gap-2">
                {!singleMode ? (
                  <>
                    ...
                    {queueAllFinished ? (
                      <Button ref={resetQueueBtnRef} caption={"Reset Queue"} disabled={queueResetting} icon={resetBtnIconCls} onClick={(e) => resetQueue()} />
                    ) : null}
                    {queueRunning ? (
                      <Button ref={stopQueueBtnRef} caption={"Stop Queue"} disabled={queueStoping} icon={stopBtnIconCls} onClick={(e) => stopQueue()} />
                    ) : null}
                  </>
                ) : (
                  ""
                                            ...
                                            {!queueRunning ? (
                                              <>
                                                {result === QueueResult.SUCCESS ? (
                                                  <Button title="Requeue" onClick={(e) => resetQueueSingle(tidx, result, toc, qState)} icon="fa fa-trash" />
                                                ) : (
                                                  <Button
                                                    onClick={(e) => {
                                                    ...
                                                    icon={`fa fa-${
                                                      result === QueueResult.INTERUPTED || result === QueueResult.FAILED ? "refresh" : "download"
                                                    }`}
                                                  />
                                                )}
                                                {DEV_MODE && !QueueResult.FINISHED && (
                                                  <DropdownMenu
                                                    className={`p-0`}
                                                    btnClassName={`px-0 py-0 pl-2 pr-2`}
                                                    data={ddToolbarData}
                                                    onSelect={(e) => onSelectDdToolbar(e, tidx, result, toc, qState)}
                                                    labelIcon={null}
                                                  />
                                                )}
