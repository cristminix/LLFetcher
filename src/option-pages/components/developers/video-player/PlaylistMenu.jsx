import {cls14, cls15,cls16,cls22,cls23,cls24, cls25,cls17,cls18,cls19,cls20,cls27} from "../../ux/cls"
             ...
             <div id={`${name}-accordion-sub`} className={`${cls25}`}> 
                     <ul className={cls22}> 
                     {
                        Object.keys(childrens).map((key, tindex) => {
                            ...
                            return <MenuItem  onSelectMenu={(a,b)=>onSelectMenu(a,b)} key={key} parentPath={path} childrens={childrens_} name={key} index={index} hasChild={item_.hasChild} title={item_.title} path={item_.path} icon={item_.iconCls} />
...
const PlaylistMenu = ({data, store, config, onSelectMenu}) => {
    ...
    <ul className={cls14}> 
    {
        Object.keys(data).map((key, index) => {
            ...
            // }
            return <MenuItem onSelectMenu={(a,b)=>onSelectMenu(a,b)} key={index} childrens={childrens} name={key} index={index} hasChild={item.hasChild} title={item.title} path={item.path} icon={item.iconCls} />
        })
    }
    </ul>
    </>)
