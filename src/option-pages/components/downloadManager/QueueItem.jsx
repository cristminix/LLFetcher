...
import { formatBytes,formatLeadingZeros} from "../../../global/fn"
...
class QueueItem extends Component{
    ...
    render(){
        const {course,dmsetup,sections, tocs, startQueueItem,logStatusBar,clearStatusBar,resetQueueItem} = this.props
        const {captionStatusRefs, videoStatusRefs} = this
        ...
        return sections.map((s, sIndex)=>{
            return tocs[s.slug].map((toc,tIndex)=>{
                ...
                return <tr key={`${refKey}`} className={trCls}>
                    ...
                    <td className={tdCls}> <div className="cursor-pointer" onMouseOut={e=>clearStatusBar()} onMouseOver={e=>logStatusBar('QueueItem.toc.title',toc.title)}>{enableFilenameIndex?`${formatLeadingZeros(number)}-${toc.title}`:toc.title}</div></td>
                    ...
                    <td className={tdCls}> <QueueItemToolbar course={course} toc={toc} logStatusBar={logStatusBar} clearStatusBar={clearStatusBar} vIndex={vIndex} resetQueueItem={resetQueueItem} startQueueItem={startQueueItem} loading={loadings[refKey]} dlStatus={vIndexStatus} finished={dmstatus.finished} interupted={dmstatus.interupted}/></td>
                </tr>
            })
        })
    }
