...
const FormRow = ({label,onChange,value})=>{
    return <div className="flex  items-center p-2 px-2">
    <div className="w-[70px]">
        <label className="font-bold">{label}</label>
    </div>
    <div className="flex-grow">
        <input  className={inputCls} value={value} onChange={e=>{onChange(e)}}/>
    </div>
</div>
}
...
const FormRowValidation = ({label, onChange, value, validationErrors, fieldname, autofocus=false, useTextArea=false, className=""}) => {
    ...
    <div className="flex-grow">
        <div className="relative">
            {
            useTextArea? <textarea  className={`${validationErrors[fieldname]?inputClsError:inputCls}  ${niceScrollbarCls} ${fieldname} ${className}`} value={value} onChange={onChange} autofocus={autofocus}></textarea>
            : <input  className={`${validationErrors[fieldname]?inputClsError:inputCls} ${fieldname} ${className}`} value={value} onChange={onChange} autofocus={autofocus}/>
...
const FormRowImageValidation = ({label, onChange, validationErrors, fieldname, className="",inputRef,imageUrl,validImage})=>{
    ...
    {
        validImage?<div className="flex-grow rounded rounded-md overflow-hidden mb-2">
            <img src={`${imageUrl}`}/>
        </div>:null
    }
        ...
        <div className="flex">
        { validationErrors[fieldname] && <ValidationErrIcon absolute="no"/>}{ validationErrors[fieldname] && <p className="text-sm text-red-600 mt-0">{validationErrors[fieldname].message}</p>}
        </div>
...
export {
    FormRow,FormRowValidation,FormRowImageValidation
}