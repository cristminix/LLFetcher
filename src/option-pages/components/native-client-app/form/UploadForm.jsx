...
const UploadForm = ({data=null, className,hideForm,updateList, formId="hs-basic-modal-upload"})=>{
    ...
    return <>
<button id="basic-modal-upload-clicker" type="button" className={btnCls} data-hs-overlay={`#${formId}`}>Open modal</button>
  ...
  <div className="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-0 transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto ]">
    <div className="flex w-[700px] flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">
      <div className="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700">
        ...
        <button type="button" id="basic-modal-upload-closer-x" onClick={e=>onCancelForm(e)} className={modalBtnCloseCls}>
    ...
    <FormRowValidation validationErrors={validationErrors} label="Title" value={title} fieldname="title"  onChange={e=>{setTitle(e.target.value)}} autofocus="yes"/>   
    <FormRowValidation validationErrors={validationErrors} useTextArea={true} label="Description" value={description} fieldname="description"  onChange={e=>{setDescription(e.target.value)}}/>   
    
    <FormRow label="Category" value={category} onChange={e=>{setCategory(e.target.value)}}/>
    <FormRow label="Tags" value={tags} onChange={e=>{setTags(e.target.value)}}/>
    <FormRow label="Video" value={video} onChange={e=>{setVideo(e.target.value)}}/>
    <FormRowImageValidation validationErrors={validationErrors} label="Thumbnail" onChange={e=>setThumbnailFile(e.target)} fieldname="thumbnail" inputRef={thumbnailRef} imageUrl={thumbnailUrl} validImage={thumbnailValid}/>
    </form>
      </div>
