import { isTimeExpired } from "@/global/class/course-api/course_fn"
import { formatLeadingZeros } from "@/global/fn"
import { QueueResult } from "@/global/class/models/Queue"
...
const downloadUsingStreamWriter = async (
  url,
  filename,
  onProgressCallback = (f) => f
) => {
  return new Promise((resolve, reject) => {
...
const downloadUsingAria2c = async (
  url,
  filename,
  course,
  onProgressCallback = (f) => f
...
const checkQueueIsAllFinished = async (courseId, tocArray = [], mQState) => {
  ...
  const recordsFiltered = records.filter(
    (rec) => rec.result === QueueResult.SUCCESS
  )
...
const downloadController = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    const filenamePrefix = enableFilenameIndex
      ? `${formatLeadingZeros(idx + 1)}-`
      : ""
    const filename = `${filenamePrefix}${toc.slug}-${selectedFmt}.${
      t == "t" ? "vtt" : "mp4"
    }`
    // const url = vttUrl
    let downloader = null
...
const downloadVtt = async (
  ...
  return await downloadController(
    vttUrl,
    idx,
    course,
    toc,
...
const selectVttUrl = (transcripts, courseId, store, defaultLang = "en") => {
  ...
  let filteredTanscripts = transcripts.filter(
