...
import { Prx, requestIdentityToken } from "@/global/class/prx"
...
const YTUploadTT = ({ store, config, pageNumber, uploadId }) => {
  ...
  const cloneForm = async (item, index) => {
    ...
    const messageClone = kind === "upload" ? `source parent "${uploadId}"` : `selected item : ${item.title}`
    if (confirm(`Are you sure want to clone from ${messageClone}`)) {
      ...
      try {
        ...
        const { data, validJson, code, text } = await Prx.post(url, requestToken, formData)
        if (validJson) {
          const { success, message } = data
          toast(message, success ? "success" : "error")
          ...
          toast(`Failed to clone id:${uploadId} server sent http ${code} ${text}`, "error")
        }
      } catch (e) {
        console.log(e)
        toast(e.toString(), "error")
  ...
  const gridOptions = {
    ...
    callbackActions: {
      edit: (item, index, options, linkCls, gridAction) => {
        return (
          <>
            <Button title="Preview" loading={false} icon="fa fa-eye" caption="" onClick={(e) => previewForm(item, index)} />
            <Button title="Clone" loading={false} icon="fa fa-copy" caption="" onClick={(e) => cloneForm(item, index)} />
            ...
            {!showForm ? (
              <>
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={(e) => cloneForm()} icon="fa fa-copy" caption="CLone Source" />
                  <Button onClick={(e) => compareForm()} icon="fa fa-filter" caption="Compare" />
                </div>
              </>
            ) : null}
          </div>
